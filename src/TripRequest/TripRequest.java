...
public class TripRequest {
	...
	public TripRequest(Airport departure, Airport arrival,
			String departureDate, String arrivalDate,
			String returnDepartureDate, String returnArrivalDate,
			boolean oneWay, boolean economySeat, boolean searchByDeparture,
			String earliestFirst, String latestFirst, String earliestSecond,
			String latestSecond) throws Exception {

		DateTimeFormatter dateParser = DateTimeFormatter
				.ofPattern("yyyy_MM_dd");
		DateTimeFormatter timeParser = DateTimeFormatter
				.ofPattern("yyyy_MM_dd HH:mm");
		...
		try {
			...
			;
			this.returnDepartureDate = LocalDate.parse(returnDepartureDate,
					dateParser);
			;
			this.returnArrivalDate = LocalDate.parse(returnArrivalDate,
					dateParser);
			;
			...
			if (!oneWay) {
				this.earliestSecond = LocalDateTime.parse(earliestSecond,
						timeParser);
				this.latestSecond = LocalDateTime.parse(latestSecond,
						timeParser);
			}
		} catch (Exception e) {
			this.isInvalid = true;
	...
	public void timeFrame(String earliest, String latest) {
		...
		try {
			...
			this.isInvalid = true;
			this.invalidMessage = "Error in time frame format, please check time frame format";
		}
	}

	public void timeFrame(String earliestFirst, String latestFirst,
			String earliestSecond, String latestSecond) {
		try {
			if (this.searchByDeparture) {
				...
				this.earliestSecond = TimeConverter.toUTCTime(
						this.arrival.latitude(), this.arrival.longitude(),
						eslocalDate);
				String lslocalDate = this.arrivalDateString() + " "
						+ latestSecond;
				...
				this.latestFirst = TimeConverter.toUTCTime(
						this.arrival.latitude(), this.arrival.longitude(),
						lflocalDate);
				String eslocalDate = this.departureDateString() + " "
						+ earliestSecond;
				this.earliestSecond = TimeConverter.toUTCTime(
						this.departure.latitude(), this.departure.longitude(),
						eslocalDate);
		 ...
		 * old code DateTimeFormatter timeParser =
		 * DateTimeFormatter.ofPattern("HH:mm"); this.earliestFirst =
		 * LocalTime.parse(earliestFirst, timeParser); this.latestFirst =
		 * LocalTime.parse(latestFirst, timeParser); this.earliestSecond =
		 * LocalTime.parse(earliestSecond, timeParser); this.latestSecond =
		 * LocalTime.parse(latestSecond, timeParser);
		 */
	}
